...
class UDADataset(object):
    ...
    def __init__(self, source, target, night, all_target, cfg):
        self.source = source
        self.target = target
        self.night = night
    ...
    def synchronized_crop(self, s1, s2, s3, s4):
    ...
    def get_rare_class_sample(self):
        ...
        i3 = np.random.choice(range(len(self.night)))
        s3 = self.night[i3]

        i4 = np.random.choice(range(len(self.all_target)))
        s4 = self.all_target[i4]
        ...
        s1, s2, s3, s4 = self.synchronized_crop(s1, s2, s3, s4)
        out = {
            **s1, 'target_img_metas': s2['img_metas'],
            'target_img': s2['img'], 'night_img_metas': s3['img_metas'],
             'night_img': s3['img'], 'all_target_img_metas': s4['img_metas'],
            'all_target_img': s4['img']
        }
