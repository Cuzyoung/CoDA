...
class HRDAEncoderDecoder(EncoderDecoder):
    ...
    def extract_unscaled_feat(self, img, levels=None):
        x = self.backbone(img, levels=levels)
        if self.with_neck:
            x = self.neck(x)
        return x

    def extract_slide_feat(self, img, levels=None):
        ...
        crop_feats = self.extract_unscaled_feat(crop_imgs, levels=levels)
    ...
    def generate_pseudo_label(self, img, img_metas, levels=None):
        self.update_debug_state()
        out = self.encode_decode(img, img_metas, levels=levels)
        if self.debug:
            self.debug_output = self.decode_head.debug_output
        return out

    def encode_decode(self, img, img_metas, upscale_pred=True, levels=None):
        ...
        for i, s in enumerate(self.scales):
            ...
            if i >= 1 and self.hr_slide_inference:
                mres_feats.append(self.extract_slide_feat(scaled_img,levels=levels))
            else:
                mres_feats.append(self.extract_unscaled_feat(scaled_img,levels=levels))
    ...
    def _forward_train_features(self, img, levels=None):
        mres_feats = []
        self.decode_head.debug_output = {}
    ...
    def forward_train(self,
        ...
        mres_feats, prob_vis = self._forward_train_features(img,levels=levels)
        for i, s in enumerate(self.scales):
