...
class DACS(UDADecorator):
    ...
    def forward_train(self,
        ...
        mini_iter = 5200
        scene_iter = 400
        scene_position = self.local_iter % mini_iter
        ...
        if not self.source_only:
            ...
            for m in self.get_ema_model().modules():
                ...
                if isinstance(m, DropPath):
                    m.training = False
            """step3: first train 12000 iters by sampling from CoT data, 
            then train rest iters by randomly sampling from target and twi_night data """
            if self.local_iter < 12000:
                """step2: first train 5200 iters by sampling from easy and twilight date,
                then tran rest iters by sampling from target and night data"""
                if scene_position < scene_iter * 3:
                    levels = 1
                    ema_logits = self.get_ema_model().generate_pseudo_label(
                    target_img, target_img_metas, levels)
                ...
                # """step1: twilight data"""
                    ...
                    for i in range(batch_size):
                        ...
                        mixed_img[i], mixed_lbl[i] = strong_transform(
                            strong_parameters,
                            data=torch.stack((img[i], night_img[i])),
                            target=torch.stack(
                                (gt_semantic_seg[i][0], pseudo_label[i])))
                        _, mixed_seg_weight[i] = strong_transform(
                            strong_parameters,
                    ...
                    if self.enable_masking and self.mask_mode.startswith('separate'):
                        masked_loss = self.mic(self.get_model(), img, img_metas,
                                            gt_semantic_seg, night_img,
                                            night_img_metas, valid_pseudo_mask,
                                            pseudo_label, pseudo_weight,levels=levels)
                        seg_debug.update(self.mic.debug_output)
                        masked_loss = add_prefix(masked_loss, 'masked')
                        masked_loss, masked_log_vars = self._parse_losses(masked_loss)
                        log_vars.update(masked_log_vars)
                ...
                ema_logits = self.get_ema_model().generate_pseudo_label(
                                all_target_img, all_target_img_metas)
                seg_debug['Target'] = self.get_ema_model().debug_output
                ...
                for i in range(batch_size):
                    ...
                    mixed_img[i], mixed_lbl[i] = strong_transform(
                            strong_parameters,
                            data=torch.stack((img[i], all_target_img[i])),
                            target=torch.stack(
                                (gt_semantic_seg[i][0], pseudo_label[i])))
                    _, mixed_seg_weight[i] = strong_transform(
                            strong_parameters,
                ...
                if self.enable_masking and self.mask_mode.startswith('separate'):
                    masked_loss = self.mic(self.get_model(), img, img_metas,
                                            gt_semantic_seg, all_target_img,
                                            all_target_img_metas, valid_pseudo_mask,
                                            pseudo_label, pseudo_weight, levels=levels)
                    seg_debug.update(self.mic.debug_output)
                    masked_loss = add_prefix(masked_loss, 'masked')
                    masked_loss, masked_log_vars = self._parse_losses(masked_loss)
